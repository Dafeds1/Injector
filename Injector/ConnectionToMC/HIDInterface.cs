...
namespace HIDInterface
{
    internal class HIDDevice
    {
        ...
        public HIDDevice(ushort VID, ushort PID, bool useAsyncReads)
        {
            ...
            if (!deviceConnected)
            {
                throw new Exception("Device with VID: 0x" + VID.ToString("X4") + " PID: 0x" + PID.ToString("X4") + "  could not be found");
            }
        }
        ...
        private void initDevice(string devicePath, bool useAsyncReads)
        {
            ...
            FS_read = new FileStream(handle_read, FileAccess.Read, capabilities.InputReportByteLength, false);
            FS_write = new FileStream(handle_write, FileAccess.Write, capabilities.OutputReportByteLength, false);
        ...
        public void write(byte[] data)  
        {
            ...
            Array.Copy(data, 0, packet, 0, data.Length);            //start at 1, as the first byte must be zero for HID report
            packet[0] = 0;
